...
#
# This script trains the NN with given hyperparameters. 
# It saves the network every 100 epoch and export the training log and costs.
#
##################################################################
...
def compute_cost(Z_out, Y, parameters, lambd, m):
	...
	cost_cross_entropy = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits_v2(logits=logits, labels=labels))
	if lambd != None:
...
def model(X_train, Y_train, X_test, Y_test, params_size, learning_rate = 0.005,
	...
	with tf.compat.v1.Session() as sess:
		...
		for epoch in range(start_epoch, num_epochs + 1):
			...
			if print_cost == True and epoch % 100 == 0:
				...
				print_with_log("Train Accuracy: {}".format(train_accuracy))
				print_with_log("Test Accuracy: {}".format(test_accuracy))

				saver.save(sess, log_path + 'save_net/save_net', global_step=epoch)
	...
	return parameters, max_test_accuracy
...
def training(train_index, log_path, csv_path):
	...
	hyperparams = {'params_size':  [4, 64, 128, 64, 32, 11], 
			   'learning_rate': 0.001, 
			   'num_epochs': 3000, 
			   'minibatch_size': 128, 
			   'lambda': 0.1}
	...
	# train the model
	parameters, max_test_accuracy = model(X_train, y_train, X_test, y_test, 
	...
	try:
		os.rename(log_path + 'log.txt', log_path + 'log #%i %.4f.txt' % (train_index, max_test_accuracy))
		os.rename(log_path + 'costs.txt', log_path + 'costs #%i.txt' % train_index)
	except FileExistsError:
		print("** Files already exist! Please check for them! **")
...
def main():
	...
	# tuning lambda for L2 regularization, from 0.0001 to 1, take logrithmic metric, do 20 times tests
	# for train_index in range(12, 21):
	# 	r = -2 * np.random.rand()
	# 	lambd = 10 ** r
	# 	_ = training(train_index, log_path, csv_path, lambd)

	# train_index = 0
	_ = training(train_index, log_path, csv_path)


csv_path = './metadata_df.csv'
log_path = './nn_model/test/'
